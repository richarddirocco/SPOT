  ...
  sidebarLayout(
    sidebarPanel(
      ...
      conditionalPanel("input.VvD_Selecter == '0'", 
          selectInput("VvD_Group", label = "Sélectionner les groupes :",
      ...
      conditionalPanel("input.VvD_Selecter == '1'", 
                      selectInput("VvD_CName", label = "Sélectionner l’espèce :",
      ...
      conditionalPanel("input.VvD_Selecter == '2'", 
          selectInput("VvD_SName", label = "Sélectionner l’espèce :",
      ...
      conditionalPanel("input.Calculate2 == '1'", 
          textInput("WV", label = "Vitesse de l’eau en m/s :", value = "0.15")),
                          
      conditionalPanel("input.Calculate2 == '1'",
          checkboxInput("WVEst", label = "Estimations du tracé :", value = TRUE)),
                          
      conditionalPanel("input.Calculate2 == '2'",
           textInput("SD", label = "Distance de nage en mètres :", value = "10")),
                          
       conditionalPanel("input.Calculate2 == '2'",
            checkboxInput("SDEst", label = "Estimations du tracé :", value = TRUE)),
...
server <- function(input, output, session){ 
  ...
  VvD_Group <- reactive({
    ...
    if(input$VvD_Selecter==1){
      tempVvD_Group <- (FishList[which(FishList$FrenchName==input$VvD_CName),"FrenchGroupName"])}
    if(input$VvD_Selecter==2){(
      tempVvD_Group <-FishList[which(FishList$ScientificName==input$VvD_SName),"FrenchGroupName"])}
    tempVvD_Group
  })
  ...
  observe({
    ...
    if(input$VvD_Selecter==1){
      scale <- FishList[which(FishList$FrenchName==input$VvD_CName),"MaxSize"]}
    if(input$VvD_Selecter==2){
  ...
  output$Plot1 <- renderPlot(
    {ggplot(data=Plot2Data(), aes(Velocity)) +
        ...
        geom_line(aes(y=X5,  colour = "Intervalle de prédiction de 95 %  "), size = 0.8, alpha=Alpha2(),  na.rm = TRUE)+
        geom_line(aes(y=X25, colour = "Intervalle de prédiction de 75 %  "), size = 0.8, alpha=Alpha2(),  na.rm = TRUE)+
        geom_line(aes(y=X50, colour = "Moyenne  "), size = 0.8, alpha=Alpha2(),  na.rm = TRUE)+
        geom_line(aes(y=X75, colour = "Intervalle de prédiction de 75 %  "), size = 0.8, alpha=Alpha2(),  na.rm = TRUE)+
        geom_line(aes(y=X95, colour = "Intervalle de prédiction de 95 %  "), size = 0.8, alpha=Alpha2(),  na.rm = TRUE)+
  ...
  output$GroupTitle2 <- renderText({
    paste("Groupe de", tolower(VvD_Group()))
  })
  ...
  output$DistanceText5 <- renderText({
    if(X5Num() < min(Plot2Data()$X5,na.rm=TRUE)){
      DT5 <- paste("2,5 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm ne peuvent pas nager contre un courant de ", WVnum(), "m/s")}
    if(X5Num() >= min(Plot2Data()$X5,na.rm=TRUE) & X5Num() < max(Plot2Data()$X5,na.rm=TRUE)){
      DT5 <- paste("2,5 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm peuvent nager sur ", signif(X5Num(), digits = 2), "m dans un courant de", WVnum(), "m/s")}
    if(X5Num() >= max(Plot2Data()$X5,na.rm=TRUE)){
      DT5 <- paste("2,5 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm peuvent nager sur ", signif(max(Plot2Data()$X5,na.rm=TRUE), digits =2), 
                   "m dans un courant de", WVnum(), "m/s")}
    DT5
  })
  
  output$DistanceText25 <- renderText({
    if(X25Num() < min(Plot2Data()$X25,na.rm=TRUE)){
      DT25 <- paste("12,5 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm ne peuvent pas nager contre un courant de ", WVnum(), "m/s")}
    if(X25Num() >= min(Plot2Data()$X25,na.rm=TRUE) & X25Num() < max(Plot2Data()$X25,na.rm=TRUE)){
      DT25 <- paste("12,5 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm peuvent nager sur ", signif(X25Num(), digits = 2), "m dans un courant de", WVnum(), "m/s")}
    if(X25Num() >= max(Plot2Data()$X25,na.rm=TRUE)){
      DT25 <- paste("12,5 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm peuvent nager sur ", signif(max(Plot2Data()$X25,na.rm=TRUE), digits =2), 
                    "m dans un courant de", WVnum(), "m/s")}
    DT25
  })
  ...
  output$DistanceText50 <- renderText({
    if(X50Num() < min(Plot2Data()$X50,na.rm=TRUE)){
      DT50 <- paste("50 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm ne peuvent pas nager contre un courant de ", WVnum(), "m/s")}
    if(X50Num() >= min(Plot2Data()$X50,na.rm=TRUE) & X50Num() < max(Plot2Data()$X50,na.rm=TRUE)){
      DT50 <- paste("50 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm peuvent nager sur ", signif(X50Num(), digits = 2), "m dans un courant de", WVnum(), "m/s")}
    if(X50Num() >= max(Plot2Data()$X50,na.rm=TRUE)){
      DT50 <- paste("50 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm peuvent nager sur ", signif(max(Plot2Data()$X50,na.rm=TRUE), digits =2), 
                    "m dans un courant de", WVnum(), "m/s")}
    DT50
  })
  ...
  output$DistanceText75 <- renderText({
    if(X75Num() < min(Plot2Data()$X75,na.rm=TRUE)){
      DT75 <- paste("87,5 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm ne peuvent pas nager contre un courant de ", WVnum(), "m/s")}
    if(X75Num() >= min(Plot2Data()$X75,na.rm=TRUE) & X75Num() < max(Plot2Data()$X75,na.rm=TRUE)){
      DT75 <- paste("87,5 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm peuvent nager sur ", signif(X75Num(), digits = 2), "m dans un courant de", WVnum(), "m/s")}
    if(X75Num() >= max(Plot2Data()$X75,na.rm=TRUE)){
      DT75 <- paste("87,5 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm peuvent nager sur ", signif(max(Plot2Data()$X75,na.rm=TRUE), digits =2), 
                    "m dans un courant de", WVnum(), "m/s")}
    DT75
  })
  ...
  output$DistanceText95 <- renderText({
    if(X95Num() < min(Plot2Data()$X95,na.rm=TRUE)){
      DT95 <- paste("97,5 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm ne peuvent pas nager contre un courant de ", WVnum(), "m/s")}
    if(X95Num() >= min(Plot2Data()$X95,na.rm=TRUE) & X95Num() < max(Plot2Data()$X95,na.rm=TRUE)){
      DT95 <- paste("97,5 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm peuvent nager sur ", signif(X95Num(), digits = 2), "m dans un courant de", WVnum(), "m/s")}
    if(X95Num() >= max(Plot2Data()$X95,na.rm=TRUE)){
      DT95 <- paste("97,5 % des ", tolower(VvD_Group()), "de", input$VvD_l, "mm peuvent nager sur ", signif(max(Plot2Data()$X95,na.rm=TRUE), digits =2), 
                    "m dans un courant de", WVnum(), "m/s")}
    DT95
  })
  ...
  output$VelocityText5 <- renderText({
    if(V5Num()  < Plot2Data()$Velocity[which.max(Plot2Data()$X5)]){
      VT5 <- paste("2,5 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm ne peuvent pas nager sur ", SDnum(), ", peu importe le courant")}
    if((V5Num() <= Plot2Data()$Velocity[which.min(Plot2Data()$X5)]) & V5Num()  >= Plot2Data()$Velocity[which.max(Plot2Data()$X5)]){
      VT5 <- paste("2,5 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm peuvent nager sur au moins ",  SDnum(), " m dans un courant de ", 
                   signif(V5Num(), digits = 2),  "m/s")}
    if(V5Num()  > Plot2Data()$Velocity[which.min(Plot2Data()$X5)]){
      VT5 <- paste("2,5 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm peuvent nager sur au moins ", SDnum(), " m dans un courant de ", 
                   signif(Plot2Data()$Velocity[which.min(Plot2Data()$X5)], digits = 2),  " m/s")}
    VT5
  })
  
  output$VelocityText25 <- renderText({
    if(V25Num()  < Plot2Data()$Velocity[which.max(Plot2Data()$X25)]){
      VT25 <- paste("12,5 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm ne peuvent pas nager sur ", SDnum(), ", peu importe le courant")}
    if((V25Num() <= Plot2Data()$Velocity[which.min(Plot2Data()$X25)]) & V25Num()  >= Plot2Data()$Velocity[which.max(Plot2Data()$X25)]){
      VT25 <- paste("12,5 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm peuvent nager sur au moins ",  SDnum(), " m dans un courant de ", 
                    signif(V25Num(), digits = 2), " m/s")}
    if(V25Num()  > Plot2Data()$Velocity[which.min(Plot2Data()$X25)]){
      VT25 <- paste("12,5 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm peuvent nager sur au moins ", SDnum(), " m dans un courant de ", 
                 signif(Plot2Data()$Velocity[which.min(Plot2Data()$X25)], digits = 2),  " m/s")}
    VT25
  })
  
  output$VelocityText50 <- renderText({
    if(V50Num()  < Plot2Data()$Velocity[which.max(Plot2Data()$X50)]){
      VT50 <- paste("50 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm ne peuvent pas nager sur ", SDnum(), ", peu importe le courant")}
    if((V50Num() <= Plot2Data()$Velocity[which.min(Plot2Data()$X50)]) & V50Num()  >= Plot2Data()$Velocity[which.max(Plot2Data()$X50)]){
      VT50 <- paste("50 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm peuvent nager sur au moins ",  SDnum(), " m dans un courant de ", 
                    signif(V50Num(), digits = 2), " m/s")}
    if(V50Num()  > Plot2Data()$Velocity[which.min(Plot2Data()$X50)]){
      VT50 <- paste("50 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm peuvent nager sur au moins ", SDnum(), " m dans un courant de ", 
                    signif(Plot2Data()$Velocity[which.min(Plot2Data()$X50)], digits = 2),  " m/s")}
    VT50
  })
  
  output$VelocityText75 <- renderText({
    if(V75Num()  < Plot2Data()$Velocity[which.max(Plot2Data()$X75)]){
      VT75 <- paste("87,5 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm ne peuvent pas nager sur ", SDnum(), ", peu importe le courant")}
    if((V75Num() <= Plot2Data()$Velocity[which.min(Plot2Data()$X75)]) & V75Num()  >= Plot2Data()$Velocity[which.max(Plot2Data()$X75)]){
      VT75 <- paste("87,5 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm peuvent nager sur au moins ",  SDnum(), " m dans un courant de ", 
                    signif(V75Num(), digits = 2), " m/s")}
    if(V75Num()  > Plot2Data()$Velocity[which.min(Plot2Data()$X75)]){
      VT75 <- paste("87,5 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm peuvent nager sur au moins ", SDnum(), " m dans un courant de ", 
                    signif(Plot2Data()$Velocity[which.min(Plot2Data()$X75)], digits = 2),  " m/s")}
    VT75
  })
  
  output$VelocityText95 <- renderText({
    if(V95Num()  < Plot2Data()$Velocity[which.max(Plot2Data()$X95)]){
      VT95 <- paste("97,5 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm ne peuvent pas nager sur ", SDnum(), ", peu importe le courant")}
    if((V95Num() <= Plot2Data()$Velocity[which.min(Plot2Data()$X95)]) & V95Num()  >= Plot2Data()$Velocity[which.max(Plot2Data()$X95)]){
      VT95 <- paste("97,5 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm peuvent nager sur au moins ",  SDnum(), " m dans un courant de ", 
                    signif(V95Num(), digits = 2), " m/s")}
    if(V95Num()  > Plot2Data()$Velocity[which.min(Plot2Data()$X95)]){
      VT95 <- paste("97,5 % des ", tolower(VvD_Group()), "de ", input$VvD_l, "mm peuvent nager sur au moins ", SDnum(), " m dans un courant de ", 
                    signif(Plot2Data()$Velocity[which.min(Plot2Data()$X95)], digits = 2),  " m/s")}
    VT95
  })
